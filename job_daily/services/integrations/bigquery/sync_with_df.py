...
def _prepare_param_value(value: any, bq_type: str) -> any:
    ...
    # Convert all complex types to strings for better reliability
    if isinstance(value, (dict, list)):
        try:
            return json.dumps(value, separators=(",", ":"))
        except Exception as e:
            app_logger.error(f"Error converting complex value to JSON: {str(e)}")
            return str(value)
    ...
    # Convert all other values to strings if they're not basic types
...
def _log_retry(retry_state: any) -> None:
    ...
    if exception:
        ...
        app_logger.debug(f"Retry attempt {attempt}: Retrying in {waiting:.2f} seconds.")
...
def _prepare_rows(
    ...
    """
    app_logger.info(
        f"Preparing rows for sync. Input data: {len(data)} rows, Existing data: {len(existing_df)} rows"
    )

    rows_to_insert = []
    rows_to_update = []

    for idx, new_row in data.iterrows():
        ...
        if existing_row.empty:
            ...
            for col in new_row.index:
                ...
                if col not in existing_row.columns or not _equals(
                    ...
                    # Log complex data types for debugging
                    if isinstance(new_val, (dict, list)) and len(str(new_val)) > 100:
                        app_logger.debug(
                            f"Complex value in row to update, column '{col}': {str(new_val)[:100]}..."
                        )
    ...
    app_logger.info(
        f"Prepared {len(rows_to_insert)} rows for insert and {len(rows_to_update)} rows for update"
    )
...
def _execute_batch_insert(
    ...
    if not rows_to_insert:
        app_logger.debug("No rows to insert, skipping batch insert.")
        return

    num_columns = len(data.columns)
    batch_size = 100  # Process 100 rows at a time
    app_logger.info(
        f"Preparing to insert {len(rows_to_insert)} rows with {num_columns} columns in batches of {batch_size}"
    )
    ...
    for batch_start in range(0, len(rows_to_insert), batch_size):
        batch_end = min(batch_start + batch_size, len(rows_to_insert))
        current_batch = rows_to_insert[batch_start:batch_end]
        app_logger.debug(
            f"Processing batch {batch_start//batch_size + 1} ({len(current_batch)} rows)"
        )
        ...
        try:
            ...
            for row_index, row in enumerate(current_batch):
                row_placeholders = []
                # Process each column
                for col in data.columns:
                    param_name = f"{col}_{batch_start + row_index}"
                    row_placeholders.append("@" + param_name)
                    param_value = _prepare_param_value(row[col], column_types[col])
                    parameters.append(
                        bigquery.ScalarQueryParameter(
                            param_name, column_types[col], param_value
                        )
                    )
                row_placeholders.append("CURRENT_TIMESTAMP()")
            ...
            # Execute the batch insert with retries
            retry_count = 0
            while retry_count < max_retries:
                try:
                    ...
                    app_logger.debug(
                        f"Executing BigQuery batch insert job for batch {batch_start//batch_size + 1} (attempt {retry_count + 1})..."
                    )
                    job = client.query(insert_query, job_config=insert_job_config)

                    # Poll for job status with a maximum timeout
                    max_wait_time = 60  # 60 seconds maximum wait time per batch
                    ...
                    while True:
                        try:
                            ...
                            if elapsed_time > max_wait_time:
                                app_logger.error(
                                    f"Batch insert job timed out after {max_wait_time} seconds"
                                )
                                raise TimeoutError(
                                    f"Batch insert job timed out after {max_wait_time} seconds"
                                )
                            ...
                            app_logger.debug(
                                f"Batch {batch_start//batch_size + 1} inserted successfully"
                            )
                            break  # Job completed successfully
                        except TimeoutError:
                            poll_count += 1
                            app_logger.debug(
                                f"Batch insert job still running... (poll #{poll_count}, elapsed time: {elapsed_time:.1f}s)"
                            )
                            ...
                            if job.state == "DONE":
                                if job.error_result:
                                    app_logger.error(f"Job failed: {job.error_result}")
                                    raise Exception(
                                        f"BigQuery job failed: {job.error_result}"
                                    )
                                break

                    # If we get here, the batch insert was successful
                    inserted_count += len(current_batch)
                    app_logger.debug(
                        f"Batch {batch_start//batch_size + 1} inserted. Total rows inserted so far: {inserted_count}/{len(rows_to_insert)}."
                    )
                    ...
                    if retry_count < max_retries:
                        app_logger.warning(
                            f"Error on attempt {retry_count} for batch {batch_start//batch_size + 1}, retrying: {str(e)}"
                        )
                        time.sleep(2**retry_count)  # Exponential backoff
                    else:
                        app_logger.error(
                            f"Failed to insert batch {batch_start//batch_size + 1} after {max_retries} attempts"
                        )
                        raise

        except Exception as e:
            app_logger.error(
                f"Error processing batch {batch_start//batch_size + 1}: {str(e)}"
            )
            app_logger.error(
                f"First row in batch: {current_batch[0] if current_batch else 'No rows'}"
            )
            raise
...
def sync_with_df(
    ...
    :return: None
    """
    app_logger.info(f"Starting sync_with_df for table {project}.{dataset}.{table}")
    app_logger.debug(f"DataFrame shape: {data.shape}, columns: {list(data.columns)}")
    app_logger.debug(f"DataFrame dtypes: {data.dtypes.to_dict()}")
    ...
    try:
        ...
        app_logger.debug("Testing BigQuery connection...")
        test_query = "SELECT 1"
        try:
            test_job = client.query(test_query)
            test_job.result(timeout=30)  # Wait up to 30 seconds for response
            app_logger.debug("BigQuery connection test successful")
        except Exception as e:
            app_logger.error(f"BigQuery connection test failed: {str(e)}")
            raise
    except Exception as e:
        app_logger.error(f"Failed to initialize BigQuery client: {str(e)}")
        raise

    table_id = f"{project}.{dataset}.{table}"
    table_ref = f"`{table_id}`"
    column_types = {col: _get_bq_field_type(col, data[col]) for col in data.columns}
    app_logger.debug(f"Column types for BigQuery: {column_types}")
    ...
    if not ref_ids:
        app_logger.debug("No reference IDs found in DataFrame; skipping update.")
        return

    app_logger.debug(f"Found {len(ref_ids)} reference IDs to process")

    # Retrieve existing rows from BigQuery
    existing_df = _get_existing_dataframe(client, table_ref, reference_id, ref_ids)
    app_logger.debug(f"Retrieved {len(existing_df)} existing rows from BigQuery")
